...
static gml_value call(gml_vm* vm, const gml_ast* n) {
    ...
    gml_value a[32];
    size_t c = n->count < 32 ? n->count : 32;
    for (size_t i = 0; i < c; i++) a[i] = eval(vm, n->items[i]);
    gml_value r = undef();

    if (!strcmp(n->text, "array_create") && c == 1) {
        double raw = num(a[0]);
        size_t count = raw > 0 ? (size_t)raw : 0;
        if (count > 4096) count = 4096;
        r = gml_value_array(count);
    ...
    else if (!strcmp(n->text, "min") && c >= 1) {
        double v = num(a[0]);
        for (size_t i = 1; i < c; i++) if (num(a[i]) < v) v = num(a[i]);
        r = gml_value_real(v);
    } else if (!strcmp(n->text, "max") && c >= 1) {
        double v = num(a[0]);
        for (size_t i = 1; i < c; i++) if (num(a[i]) > v) v = num(a[i]);
        r = gml_value_real(v);
    } else if (!strcmp(n->text, "clamp") && c == 3) {
        double x = num(a[0]), lo = num(a[1]), hi = num(a[2]);
        r = gml_value_real(x < lo ? lo : (x > hi ? hi : x));
    ...
    else if (!strcmp(n->text, "point_distance") && c == 4) {
        double dx = num(a[2]) - num(a[0]), dy = num(a[3]) - num(a[1]);
        r = gml_value_real(sqrt(dx * dx + dy * dy));
    } else if (!strcmp(n->text, "point_direction") && c == 4) {
        double dx = num(a[2]) - num(a[0]), dy = num(a[1]) - num(a[3]);
        double angle = atan2(dy, dx) * 180.0 / 3.14159265358979323846;
        if (angle < 0) angle += 360.0;
        r = gml_value_real(angle);
    } else if (!strcmp(n->text, "string") && c == 1) {
        if (a[0].kind == GML_V_STRING) r = gml_value_string(text_of(a[0]));
        else if (a[0].kind == GML_V_BOOL) r = gml_value_string(a[0].boolean ? "1" : "0");
        else r = number_text(num(a[0]));
    } else if (!strcmp(n->text, "string_length") && c == 1) {
        r = gml_value_real((double)strlen(text_of(a[0])));
    } else if (!strcmp(n->text, "string_char_at") && c == 2) {
        const char* s = text_of(a[0]);
        int pos = (int)num(a[1]);
        if (pos >= 1 && (size_t)pos <= strlen(s)) {
            char ch[2] = {s[pos - 1], 0};
            r = gml_value_string(ch);
        } else r = gml_value_string("");
    } else if (!strcmp(n->text, "string_copy") && c == 3) {
        const char* s = text_of(a[0]);
        int start = (int)num(a[1]), len = (int)num(a[2]);
        ...
        if (part) {
            memcpy(part, s + begin, (size_t)len);
            part[len] = 0;
            r = gml_value_string(part);
            free(part);
        }
    } else if (!strcmp(n->text, "string_pos") && c == 2) {
        const char* needle = text_of(a[0]);
        const char* haystack = text_of(a[1]);
        const char* found = (needle[0] ? strstr(haystack, needle) : haystack);
        r = gml_value_real(found ? (double)(found - haystack + 1) : 0);
    ...
    else if (!strcmp(n->text, "string_lower") && c == 1) {
        const char* s = text_of(a[0]);
        size_t len = strlen(s);
    ...
    } else if (!strcmp(n->text, "string_upper") && c == 1) {
        const char* s = text_of(a[0]);
        size_t len = strlen(s);
        char* buf = malloc(len + 1);
        if (buf) {
    ...
    } else if (!strcmp(n->text, "string_delete") && c == 3) {
        const char* s = text_of(a[0]);
        int start = (int)num(a[1]), del = (int)num(a[2]);
        size_t len = strlen(s);
        if (start < 1) start = 1;
        if (del < 0) del = 0;
        size_t begin = (size_t)(start - 1);
        if (begin > len) begin = len;
        if ((size_t)del > len - begin) del = (int)(len - begin);
        char* buf = malloc(len - (size_t)del + 1);
        if (buf) {
            memcpy(buf, s, begin);
            memcpy(buf + begin, s + begin + del, len - begin - (size_t)del);
            buf[len - (size_t)del] = 0;
            r = gml_value_string(buf);
            free(buf);
        }
    } else if (!strcmp(n->text, "string_insert") && c == 3) {
        const char* ins = text_of(a[0]);
        const char* s = text_of(a[1]);
        int pos = (int)num(a[2]);
        size_t ni = strlen(ins), ns = strlen(s);
        if (pos < 1) pos = 1;
        size_t at = (size_t)(pos - 1);
        if (at > ns) at = ns;
        char* buf = malloc(ni + ns + 1);
        if (buf) {
            memcpy(buf, s, at);
            memcpy(buf + at, ins, ni);
            memcpy(buf + at + ni, s + at, ns - at + 1);
            r = gml_value_string(buf);
            free(buf);
        }
    } else if (!strcmp(n->text, "string_replace_all") && c == 3) {
        ...
        if (nf == 0) r = gml_value_string(src);
        else {
    ...
    } else if (!strcmp(n->text, "string_repeat") && c == 2) {
        const char* s = text_of(a[0]);
        int times = (int)num(a[1]);
        if (times <= 0) r = gml_value_string("");
        else {
            size_t slen = strlen(s);
            char* buf = malloc(slen * (size_t)times + 1);
            if (buf) {
                buf[0] = 0;
    ...
    } else if (!strcmp(n->text, "string_letters") && c == 1) {
        const char* s = text_of(a[0]);
        size_t len = strlen(s);
        char* buf = malloc(len + 1);
        if (buf) {
    ...
    } else if (!strcmp(n->text, "string_digits") && c == 1) {
        const char* s = text_of(a[0]);
        size_t len = strlen(s);
        char* buf = malloc(len + 1);
        if (buf) {
    ...
    } else if (!strcmp(n->text, "string_count") && c == 2) {
        const char* sub = text_of(a[0]);
        const char* s = text_of(a[1]);
        size_t sublen = strlen(sub);
        int total = 0;
        if (sublen > 0) {
            for (const char* p = s; (p = strstr(p, sub)); p += sublen) total++;
        }
        r = gml_value_real((double)total);
    } else if (!strcmp(n->text, "string_replace") && c == 3) {
        const char* s = text_of(a[0]);
        const char* sub = text_of(a[1]);
        const char* newsub = text_of(a[2]);
        char* p = sub[0] ? strstr(s, sub) : NULL;
        if (p) {
            size_t pre = (size_t)(p - s), slen = strlen(sub), nlen = strlen(newsub), rlen = strlen(p + slen);
            char* buf = malloc(pre + nlen + rlen + 1);
            if (buf) {
                memcpy(buf, s, pre);
                memcpy(buf + pre, newsub, nlen);
                memcpy(buf + pre + nlen, p + slen, rlen + 1);
                r = gml_value_string(buf);
                free(buf);
            } else r = gml_value_string(s);
        } else r = gml_value_string(s);
    } else if (!strcmp(n->text, "chr") && c == 1) {
        char ch[2] = {(char)num(a[0]), 0};
        r = gml_value_string(ch);
    } else if (!strcmp(n->text, "ord") && c == 1) {
        const char* s = text_of(a[0]);
        r = gml_value_real(s && s[0] ? (unsigned char)s[0] : 0);
    } else if (!strcmp(n->text, "real") && c == 1) {
        if (a[0].kind == GML_V_REAL) r = gml_value_real(a[0].real);
        else if (a[0].kind == GML_V_BOOL) r = gml_value_real(a[0].boolean ? 1 : 0);
        else if (a[0].kind == GML_V_STRING && a[0].string) r = gml_value_real(atof(a[0].string));
        ...
        r = gml_value_string(buf);
    } else if ((!strcmp(n->text, "string_byte_at") || !strcmp(n->text, "string_ord_at")) && c == 2) {
        const char* s = text_of(a[0]);
        int pos = (int)num(a[1]);
        size_t len = strlen(s);
        r = gml_value_real((pos >= 1 && (size_t)pos <= len) ? (unsigned char)s[pos - 1] : 0);
    } else if (!strcmp(n->text, "string_byte_length") && c == 1) {
        r = gml_value_real((double)strlen(text_of(a[0])));
    ...
    else if (!strcmp(n->text, "logn") && c == 2) { double b = num(a[0]), v = num(a[1]); r = gml_value_real((b > 0 && b != 1 && v > 0) ? log(v) / log(b) : 0); }
    else if (!strcmp(n->text, "exp") && c == 1) r = gml_value_real(exp(num(a[0])));
    else if (!strcmp(n->text, "frac") && c == 1) { double v = num(a[0]); r = gml_value_real(v - floor(v)); }
    else if (!strcmp(n->text, "dsin") && c == 1) r = gml_value_real(sin(num(a[0]) * 3.14159265358979323846 / 180.0));
    else if (!strcmp(n->text, "dcos") && c == 1) r = gml_value_real(cos(num(a[0]) * 3.14159265358979323846 / 180.0));
    else if (!strcmp(n->text, "dtan") && c == 1) r = gml_value_real(tan(num(a[0]) * 3.14159265358979323846 / 180.0));
    else if (!strcmp(n->text, "darcsin") && c == 1) r = gml_value_real(asin(num(a[0])) * 180.0 / 3.14159265358979323846);
    else if (!strcmp(n->text, "darccos") && c == 1) r = gml_value_real(acos(num(a[0])) * 180.0 / 3.14159265358979323846);
    else if (!strcmp(n->text, "darctan") && c == 1) r = gml_value_real(atan(num(a[0])) * 180.0 / 3.14159265358979323846);
    else if (!strcmp(n->text, "arctan2") && c == 2) r = gml_value_real(atan2(num(a[0]), num(a[1])));
    else if (!strcmp(n->text, "median") && c >= 1) {
        double vals[32]; size_t nvals = c > 32 ? 32 : c;
        for (size_t i = 0; i < nvals; i++) vals[i] = num(a[i]);
        for (size_t i = 0; i < nvals; i++) for (size_t j = i + 1; j < nvals; j++) if (vals[i] > vals[j]) { double tmp = vals[i]; vals[i] = vals[j]; vals[j] = tmp; }
        r = gml_value_real(vals[nvals / 2]);
    } else if (!strcmp(n->text, "mean") && c >= 1) {
        double sum = 0;
    ...
    } else if ((!strcmp(n->text, "dot_product") || !strcmp(n->text, "dot_product_2d")) && c == 4) {
    ...
    } else if ((!strcmp(n->text, "make_colour_rgb") || !strcmp(n->text, "make_color_rgb")) && c == 3) {
        int red = (int)num(a[0]), green = (int)num(a[1]), blue = (int)num(a[2]);
        if (red < 0) red = 0; if (red > 255) red = 255;
        if (green < 0) green = 0; if (green > 255) green = 255;
        if (blue < 0) blue = 0; if (blue > 255) blue = 255;
        r = gml_value_real((double)(red | (green << 8) | (blue << 16)));
    } else if ((!strcmp(n->text, "colour_get_red") || !strcmp(n->text, "color_get_red")) && c == 1) {
        int col = (int)num(a[0]);
        r = gml_value_real((double)(col & 0xFF));
    } else if ((!strcmp(n->text, "colour_get_green") || !strcmp(n->text, "color_get_green")) && c == 1) {
        int col = (int)num(a[0]);
        r = gml_value_real((double)((col >> 8) & 0xFF));
    } else if ((!strcmp(n->text, "colour_get_blue") || !strcmp(n->text, "color_get_blue")) && c == 1) {
        int col = (int)num(a[0]);
        r = gml_value_real((double)((col >> 16) & 0xFF));
    } else if ((!strcmp(n->text, "colour_get_hue") || !strcmp(n->text, "color_get_hue")) && c == 1) {
        int col = (int)num(a[0]);
        double rd = (col & 0xFF) / 255.0, gd = ((col >> 8) & 0xFF) / 255.0, bd = ((col >> 16) & 0xFF) / 255.0;
        double maxv = rd > gd ? (rd > bd ? rd : bd) : (gd > bd ? gd : bd);
        double minv = rd < gd ? (rd < bd ? rd : bd) : (gd < bd ? gd : bd);
        double delta = maxv - minv, h = 0;
        if (delta > 0) {
            if (maxv == rd) h = fmod((gd - bd) / delta, 6.0);
            else if (maxv == gd) h = (bd - rd) / delta + 2.0;
            else h = (rd - gd) / delta + 4.0;
            h *= 60.0;
            if (h < 0) h += 360.0;
        }
        r = gml_value_real((double)((int)(h * 255.0 / 360.0)));
    } else if ((!strcmp(n->text, "colour_get_saturation") || !strcmp(n->text, "color_get_saturation")) && c == 1) {
        int col = (int)num(a[0]);
        double rd = (col & 0xFF) / 255.0, gd = ((col >> 8) & 0xFF) / 255.0, bd = ((col >> 16) & 0xFF) / 255.0;
        double maxv = rd > gd ? (rd > bd ? rd : bd) : (gd > bd ? gd : bd);
        double minv = rd < gd ? (rd < bd ? rd : bd) : (gd < bd ? gd : bd);
        double s = maxv > 0 ? (maxv - minv) / maxv : 0;
        r = gml_value_real((double)((int)(s * 255.0)));
    } else if ((!strcmp(n->text, "colour_get_value") || !strcmp(n->text, "color_get_value")) && c == 1) {
        int col = (int)num(a[0]);
        double rd = (col & 0xFF) / 255.0, gd = ((col >> 8) & 0xFF) / 255.0, bd = ((col >> 16) & 0xFF) / 255.0;
        double maxv = rd > gd ? (rd > bd ? rd : bd) : (gd > bd ? gd : bd);
        r = gml_value_real((double)((int)(maxv * 255.0)));
    ...
    for (size_t i = 0; i < c; i++) gml_value_free(&a[i]);
    return r;
}
