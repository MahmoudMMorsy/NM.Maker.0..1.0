import { ActionDefinition } from "./actionLibrary";

export const EXTERNAL_ACTIONS: ActionDefinition[] = [
  {
    id: "ext_move_999_1",
    name: "Move",
    description: "Move action",
    category: "move",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Move
// Parameters: ${keys}
console.log("Executed legacy action: Move", params);
`;
    }
  },
  {
    id: "ext_move_101",
    name: "Move Fixed",
    description: "Start moving in a direction",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Move Fixed
if (this.motionSet) this.motionSet(params.dir || 'stop', params.spd || 0);
`;
    }
  },
  {
    id: "ext_move_102",
    name: "Move Free",
    description: "Set direction and speed of motion",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Move Free
this.direction = params.dir || 0;
this.speed = params.spd || 0;
this.dx = Math.cos(this.direction * Math.PI / 180) * this.speed;
this.dy = -Math.sin(this.direction * Math.PI / 180) * this.speed;
`;
    }
  },
  {
    id: "ext_move_105",
    name: "Move Towards",
    description: "Move towards point (@0,@1)",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Move Towards
var dist = Math.hypot(params.tx - this.x, params.ty - this.y);
if (dist > params.spd) {
  var angle = Math.atan2(params.ty - this.y, params.tx - this.x);
  this.dx = Math.cos(angle) * params.spd;
  this.dy = Math.sin(angle) * params.spd;
} else {
  this.x = params.tx; this.y = params.ty; this.dx = 0; this.dy = 0;
}
`;
    }
  },
  {
    id: "ext_move_103",
    name: "Speed Horizontal",
    description: "Set the horizontal speed",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Speed Horizontal
this.dx = params.spd || 0;
`;
    }
  },
  {
    id: "ext_move_104",
    name: "Speed Vertical",
    description: "Set the vertical speed",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Speed Vertical
this.dy = params.spd || 0;
`;
    }
  },
  {
    id: "ext_move_107",
    name: "Set Gravity",
    description: "Set the gravity",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Set Gravity
this.gravity = params.val || 0;
`;
    }
  },
  {
    id: "ext_move_113",
    name: "Reverse Horizontal",
    description: "Reverse horizontal direction",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Reverse Horizontal
this.dx = -this.dx;
`;
    }
  },
  {
    id: "ext_move_114",
    name: "Reverse Vertical",
    description: "Reverse vertical direction",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Reverse Vertical
this.dy = -this.dy;
`;
    }
  },
  {
    id: "ext_move_108",
    name: "Set Friction",
    description: "Set the friction",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Set Friction
this.friction = params.val || 0;
`;
    }
  },
  {
    id: "ext_move_999_2",
    name: "-------------",
    description: "------------- action",
    category: "move",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: -------------
// Parameters: ${keys}
console.log("Executed legacy action: -------------", params);
`;
    }
  },
  {
    id: "ext_move_999_3_2",
    name: "Jump",
    description: "Jump action",
    category: "move",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Jump
// Parameters: ${keys}
console.log("Executed legacy action: Jump", params);
`;
    }
  },
  {
    id: "ext_move_109",
    name: "Jump to Position",
    description: "Jump to position (@0,@1)",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Jump to Position
this.x = params.x || 0;
this.y = params.y || 0;
`;
    }
  },
  {
    id: "ext_move_110",
    name: "Jump to Start",
    description: "Jump to the start position",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Jump to Start
this.x = this.startX || 0;
this.y = this.startY || 0;
`;
    }
  },
  {
    id: "ext_move_111",
    name: "Jump to Random",
    description: "Jump to a random position",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Jump to Random
this.x = Math.random() * ((window.ROOM_W || 320) - (this.w || 16));
this.y = Math.random() * ((window.ROOM_H || 240) - (this.h || 16));
`;
    }
  },
  {
    id: "ext_move_117",
    name: "Align to Grid",
    description: "Align to a grid of @0 by @1",
    category: "move",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAAZElEQVR4nO3W0QnAMAgE0BupIzpIhnEVJzG4QNqUk9BykfsISHiIHwGA7ExiXXUyIugxsz2Au1MjAAWAa2zdBaAA6tG3+ccEBNAOCKAdEICVbwGquSOPAUe/5XcN3SWAAAIcB0xhhXWvLuaPxQAAAABJRU5ErkJggg==",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Align to Grid
this.x = Math.round(this.x / (params.hs || 16)) * (params.hs || 16);
this.y = Math.round(this.y / (params.vs || 16)) * (params.vs || 16);
`;
    }
  },
  {
    id: "ext_move_112",
    name: "Wrap Screen",
    description: "Wrap @0 when outside",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Wrap Screen
var mar = params.mar || 0;
var rw = window.ROOM_W || 320;
var rh = window.ROOM_H || 240;
if (this.x < -mar) this.x = rw + mar;
if (this.x > rw + mar) this.x = -mar;
if (this.y < -mar) this.y = rh + mar;
if (this.y > rh + mar) this.y = -mar;
`;
    }
  },
  {
    id: "ext_move_116",
    name: "Move to Contact",
    description: "Move to contact in direction @0",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Move to Contact
// Parameters: ${keys}
console.log("Executed legacy action: Move to Contact", params);
`;
    }
  },
  {
    id: "ext_move_115",
    name: "Bounce",
    description: "Bounce against @1",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Bounce
var oldX = this.x; var oldY = this.y;
this.x += this.dx;
if (this.checkCol && this.checkCol(window.currentRoomMap, window.currentRoomWidth)) this.dx = -this.dx;
this.x = oldX;
this.y += this.dy;
if (this.checkCol && this.checkCol(window.currentRoomMap, window.currentRoomWidth)) this.dy = -this.dy;
this.y = oldY;
`;
    }
  },
  {
    id: "ext_move_999_4",
    name: "-------------",
    description: "------------- action",
    category: "move",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: -------------
// Parameters: ${keys}
console.log("Executed legacy action: -------------", params);
`;
    }
  },
  {
    id: "ext_move_999_5_3",
    name: "Paths",
    description: "Paths action",
    category: "move",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Paths
// Parameters: ${keys}
console.log("Executed legacy action: Paths", params);
`;
    }
  },
  {
    id: "ext_move_119",
    name: "Path",
    description: "Path action",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Path
// Parameters: ${keys}
console.log("Executed legacy action: Path", params);
`;
    }
  },
  {
    id: "ext_move_124",
    name: "Path_End",
    description: "Path_End action",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Path_End
// Parameters: ${keys}
console.log("Executed legacy action: Path_End", params);
`;
    }
  },
  {
    id: "ext_move_122",
    name: "Path_Position",
    description: "Path_Position action",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Path_Position
// Parameters: ${keys}
console.log("Executed legacy action: Path_Position", params);
`;
    }
  },
  {
    id: "ext_move_123",
    name: "Path_Speed",
    description: "Path_Speed action",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Path_Speed
// Parameters: ${keys}
console.log("Executed legacy action: Path_Speed", params);
`;
    }
  },
  {
    id: "ext_move_999_6",
    name: "-------------",
    description: "------------- action",
    category: "move",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: -------------
// Parameters: ${keys}
console.log("Executed legacy action: -------------", params);
`;
    }
  },
  {
    id: "ext_move_999_7_4",
    name: "Steps",
    description: "Steps action",
    category: "move",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Steps
// Parameters: ${keys}
console.log("Executed legacy action: Steps", params);
`;
    }
  },
  {
    id: "ext_move_120",
    name: "Step_Linear",
    description: "Step_Linear action",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Step_Linear
// Parameters: ${keys}
console.log("Executed legacy action: Step_Linear", params);
`;
    }
  },
  {
    id: "ext_move_121",
    name: "Step_Potential",
    description: "Step_Potential action",
    category: "move",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Step_Potential
// Parameters: ${keys}
console.log("Executed legacy action: Step_Potential", params);
`;
    }
  },
  {
    id: "ext_main1_999_1",
    name: "Objects",
    description: "Objects action",
    category: "main1",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Objects
// Parameters: ${keys}
console.log("Executed legacy action: Objects", params);
`;
    }
  },
  {
    id: "ext_main1_201",
    name: "Create Instance",
    description: "Create instance of object @0",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Create Instance
var nx = (params.rel ? this.x : 0) + (params.x || 0);
var ny = (params.rel ? this.y : 0) + (params.y || 0);
if (window.instances) {
  var def = window.GAME_DATA?.objects.find(o => o.name === params.obj);
  if (def) window.instances.push(new GMObject(nx, ny, def));
}
`;
    }
  },
  {
    id: "ext_main1_206",
    name: "Create Moving",
    description: "Create moving instance of @0",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Create Moving
// Parameters: ${keys}
console.log("Executed legacy action: Create Moving", params);
`;
    }
  },
  {
    id: "ext_main1_207",
    name: "Create Random",
    description: "Create instance of random object",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Create Random
// Parameters: ${keys}
console.log("Executed legacy action: Create Random", params);
`;
    }
  },
  {
    id: "ext_main1_202",
    name: "Change Instance",
    description: "Change instance into @0",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Change Instance
var def = window.GAME_DATA?.objects.find(o => o.name === params.obj);
if (def) {
  this.def = def;
  if (this.resolveSize) this.resolveSize();
  if (params.ev && this.triggerEvent) this.triggerEvent('create');
}
`;
    }
  },
  {
    id: "ext_main1_203",
    name: "Destroy Instance",
    description: "Destroy the instance",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Destroy Instance
this.dead = true;
`;
    }
  },
  {
    id: "ext_main1_204",
    name: "Destroy at Position",
    description: "Destroy instances at a position",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Destroy at Position
// Parameters: ${keys}
console.log("Executed legacy action: Destroy at Position", params);
`;
    }
  },
  {
    id: "ext_main1_999_2",
    name: "--------------",
    description: "-------------- action",
    category: "main1",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: --------------
// Parameters: ${keys}
console.log("Executed legacy action: --------------", params);
`;
    }
  },
  {
    id: "ext_main1_999_3",
    name: "Sprite",
    description: "Sprite action",
    category: "main1",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Sprite
// Parameters: ${keys}
console.log("Executed legacy action: Sprite", params);
`;
    }
  },
  {
    id: "ext_main1_541",
    name: "Change Sprite",
    description: "Change sprite into @0",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Change Sprite
// Parameters: ${keys}
console.log("Executed legacy action: Change Sprite", params);
`;
    }
  },
  {
    id: "ext_main1_542",
    name: "Transform_Sprite",
    description: "Transform_Sprite action",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Transform_Sprite
// Parameters: ${keys}
console.log("Executed legacy action: Transform_Sprite", params);
`;
    }
  },
  {
    id: "ext_main1_543",
    name: "Color_Sprite",
    description: "Color_Sprite action",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Color_Sprite
// Parameters: ${keys}
console.log("Executed legacy action: Color_Sprite", params);
`;
    }
  },
  {
    id: "ext_main1_205",
    name: "Set_Sprite_Old",
    description: "Set_Sprite_Old action",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Set_Sprite_Old
// Parameters: ${keys}
console.log("Executed legacy action: Set_Sprite_Old", params);
`;
    }
  },
  {
    id: "ext_main1_999_4",
    name: "-----------",
    description: "----------- action",
    category: "main1",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: -----------
// Parameters: ${keys}
console.log("Executed legacy action: -----------", params);
`;
    }
  },
  {
    id: "ext_main1_999_5",
    name: "Sounds",
    description: "Sounds action",
    category: "main1",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Sounds
// Parameters: ${keys}
console.log("Executed legacy action: Sounds", params);
`;
    }
  },
  {
    id: "ext_main1_211",
    name: "Play Sound",
    description: "Play sound @0",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Play Sound
if (window.GM82Audio) window.GM82Audio.play_sfx(params.snd || '', params.loop || false);
`;
    }
  },
  {
    id: "ext_main1_212",
    name: "Stop Sound",
    description: "Stop sound @0",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Stop Sound
if (window.GM82Audio) window.GM82Audio.stop_music();
`;
    }
  },
  {
    id: "ext_main1_213",
    name: "Check Sound",
    description: "If sound @0 is @Nplaying",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Check Sound
// Parameters: ${keys}
console.log("Executed legacy action: Check Sound", params);
`;
    }
  },
  {
    id: "ext_main1_999_6",
    name: "----------",
    description: "---------- action",
    category: "main1",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: ----------
// Parameters: ${keys}
console.log("Executed legacy action: ----------", params);
`;
    }
  },
  {
    id: "ext_main1_999_7",
    name: "Rooms",
    description: "Rooms action",
    category: "main1",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Rooms
// Parameters: ${keys}
console.log("Executed legacy action: Rooms", params);
`;
    }
  },
  {
    id: "ext_main1_221",
    name: "Previous Room",
    description: "Go to previous room",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Previous Room
if (window.gotoRoom) window.gotoRoom(window.currentRoomIdx - 1);
`;
    }
  },
  {
    id: "ext_main1_222",
    name: "Next Room",
    description: "Go to next room",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Next Room
if (window.gotoRoom) window.gotoRoom(window.currentRoomIdx + 1);
`;
    }
  },
  {
    id: "ext_main1_223",
    name: "Restart Room",
    description: "Restart the current room",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Restart Room
if (window.restartRoom) window.restartRoom();
`;
    }
  },
  {
    id: "ext_main1_224",
    name: "Different Room",
    description: "Go to room @0",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Different Room
if (window.gotoRoom) window.gotoRoom(params.rm);
`;
    }
  },
  {
    id: "ext_main1_225",
    name: "Check Previous",
    description: "If previous room exists",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Check Previous
// Parameters: ${keys}
console.log("Executed legacy action: Check Previous", params);
`;
    }
  },
  {
    id: "ext_main1_226",
    name: "Check Next",
    description: "If next room exists",
    category: "main1",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Check Next
// Parameters: ${keys}
console.log("Executed legacy action: Check Next", params);
`;
    }
  },
  {
    id: "ext_main2_999_1",
    name: "Timing",
    description: "Timing action",
    category: "main2",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Timing
// Parameters: ${keys}
console.log("Executed legacy action: Timing", params);
`;
    }
  },
  {
    id: "ext_main2_301",
    name: "Set Alarm",
    description: "Set @1 @rto @0",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Set Alarm
this.alarms = this.alarms || {};
this.alarms[params.id || '0'] = params.steps || 30;
`;
    }
  },
  {
    id: "ext_main2_302",
    name: "Sleep",
    description: "Sleep @0 milliseconds",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Sleep
// Parameters: ${keys}
console.log("Executed legacy action: Sleep", params);
`;
    }
  },
  {
    id: "ext_main2_303",
    name: "Set TimeLine (OLD)",
    description: "Set TimeLine (OLD) action",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Set TimeLine (OLD)
// Parameters: ${keys}
console.log("Executed legacy action: Set TimeLine (OLD)", params);
`;
    }
  },
  {
    id: "ext_main2_305",
    name: "Set TimeLine",
    description: "Set TimeLine action",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Set TimeLine
// Parameters: ${keys}
console.log("Executed legacy action: Set TimeLine", params);
`;
    }
  },
  {
    id: "ext_main2_304",
    name: "Position TimeLine",
    description: "Position TimeLine action",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Position TimeLine
// Parameters: ${keys}
console.log("Executed legacy action: Position TimeLine", params);
`;
    }
  },
  {
    id: "ext_main2_309",
    name: "Speed TimeLine",
    description: "Speed TimeLine action",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Speed TimeLine
// Parameters: ${keys}
console.log("Executed legacy action: Speed TimeLine", params);
`;
    }
  },
  {
    id: "ext_main2_306",
    name: "Start TimeLine",
    description: "Start TimeLine action",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Start TimeLine
// Parameters: ${keys}
console.log("Executed legacy action: Start TimeLine", params);
`;
    }
  },
  {
    id: "ext_main2_307",
    name: "Pause TimeLine",
    description: "Pause TimeLine action",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Pause TimeLine
// Parameters: ${keys}
console.log("Executed legacy action: Pause TimeLine", params);
`;
    }
  },
  {
    id: "ext_main2_308",
    name: "Stop TimeLine",
    description: "Stop TimeLine action",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Stop TimeLine
// Parameters: ${keys}
console.log("Executed legacy action: Stop TimeLine", params);
`;
    }
  },
  {
    id: "ext_main2_999_2",
    name: "----------",
    description: "---------- action",
    category: "main2",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: ----------
// Parameters: ${keys}
console.log("Executed legacy action: ----------", params);
`;
    }
  },
  {
    id: "ext_main2_999_3",
    name: "Info",
    description: "Info action",
    category: "main2",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Info
// Parameters: ${keys}
console.log("Executed legacy action: Info", params);
`;
    }
  },
  {
    id: "ext_main2_321",
    name: "Display Message",
    description: "Display a message",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Display Message
window.alert(params.msg || '');
`;
    }
  },
  {
    id: "ext_main2_322",
    name: "Show Info",
    description: "Show the game info",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Show Info
// Parameters: ${keys}
console.log("Executed legacy action: Show Info", params);
`;
    }
  },
  {
    id: "ext_main2_323",
    name: "Show_Video",
    description: "Show_Video action",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Show_Video
// Parameters: ${keys}
console.log("Executed legacy action: Show_Video", params);
`;
    }
  },
  {
    id: "ext_main2_324",
    name: "Splash_Text",
    description: "Splash_Text action",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Splash_Text
// Parameters: ${keys}
console.log("Executed legacy action: Splash_Text", params);
`;
    }
  },
  {
    id: "ext_main2_325",
    name: "Splash_Image",
    description: "Splash_Image action",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Splash_Image
// Parameters: ${keys}
console.log("Executed legacy action: Splash_Image", params);
`;
    }
  },
  {
    id: "ext_main2_326",
    name: "Splash_Web",
    description: "Splash_Web action",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Splash_Web
// Parameters: ${keys}
console.log("Executed legacy action: Splash_Web", params);
`;
    }
  },
  {
    id: "ext_main2_327",
    name: "Splash_Video",
    description: "Splash_Video action",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Splash_Video
// Parameters: ${keys}
console.log("Executed legacy action: Splash_Video", params);
`;
    }
  },
  {
    id: "ext_main2_328",
    name: "Splash_Settings",
    description: "Splash_Settings action",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Splash_Settings
// Parameters: ${keys}
console.log("Executed legacy action: Splash_Settings", params);
`;
    }
  },
  {
    id: "ext_main2_999_4",
    name: "---------",
    description: "--------- action",
    category: "main2",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: ---------
// Parameters: ${keys}
console.log("Executed legacy action: ---------", params);
`;
    }
  },
  {
    id: "ext_main2_999_5",
    name: "Game",
    description: "Game action",
    category: "main2",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Game
// Parameters: ${keys}
console.log("Executed legacy action: Game", params);
`;
    }
  },
  {
    id: "ext_main2_331",
    name: "Restart Game",
    description: "Restart the game",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Restart Game
window.location.reload();
`;
    }
  },
  {
    id: "ext_main2_332",
    name: "End Game",
    description: "End the game",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// End Game
window.gameOver = true;
`;
    }
  },
  {
    id: "ext_main2_333",
    name: "Save Game",
    description: "Save the game",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Save Game
// Parameters: ${keys}
console.log("Executed legacy action: Save Game", params);
`;
    }
  },
  {
    id: "ext_main2_334",
    name: "Load Game",
    description: "Load the game",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Load Game
// Parameters: ${keys}
console.log("Executed legacy action: Load Game", params);
`;
    }
  },
  {
    id: "ext_main2_999_6",
    name: "-----------------",
    description: "----------------- action",
    category: "main2",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAAT0lEQVR4nO3WzQkAMAiD0YzuaK7iJJYu0B9QSuEz5JbDOypJ2dnUOvMyIsprZncAdy8tAAAAAAAAAAAAgL8Ac9zRY8DTt3w36A4AAACeAwbq3OtjR9lqIQAAAABJRU5ErkJggg==",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: -----------------
// Parameters: ${keys}
console.log("Executed legacy action: -----------------", params);
`;
    }
  },
  {
    id: "ext_main2_999_7",
    name: "Resources",
    description: "Resources action",
    category: "main2",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Resources
// Parameters: ${keys}
console.log("Executed legacy action: Resources", params);
`;
    }
  },
  {
    id: "ext_main2_803",
    name: "Replace_Sprite",
    description: "Replace_Sprite action",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Replace_Sprite
// Parameters: ${keys}
console.log("Executed legacy action: Replace_Sprite", params);
`;
    }
  },
  {
    id: "ext_main2_804",
    name: "Replace_Sound",
    description: "Replace_Sound action",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Replace_Sound
// Parameters: ${keys}
console.log("Executed legacy action: Replace_Sound", params);
`;
    }
  },
  {
    id: "ext_main2_805",
    name: "Replace_Background",
    description: "Replace_Background action",
    category: "main2",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Replace_Background
// Parameters: ${keys}
console.log("Executed legacy action: Replace_Background", params);
`;
    }
  },
  {
    id: "ext_control_999_1",
    name: "Questions",
    description: "Questions action",
    category: "control",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Questions
// Parameters: ${keys}
console.log("Executed legacy action: Questions", params);
`;
    }
  },
  {
    id: "ext_control_401",
    name: "Check Empty",
    description: "If a position is collision free",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Check Empty
// Parameters: ${keys}
console.log("Executed legacy action: Check Empty", params);
`;
    }
  },
  {
    id: "ext_control_402",
    name: "Check Collision",
    description: "If there is a collision at a position",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Check Collision
// Parameters: ${keys}
console.log("Executed legacy action: Check Collision", params);
`;
    }
  },
  {
    id: "ext_control_403",
    name: "Check Object",
    description: "If there is an object at a position",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Check Object
// Parameters: ${keys}
console.log("Executed legacy action: Check Object", params);
`;
    }
  },
  {
    id: "ext_control_404",
    name: "Test Instance Count",
    description: "If the number of instances is a value",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Test Instance Count
// Parameters: ${keys}
console.log("Executed legacy action: Test Instance Count", params);
`;
    }
  },
  {
    id: "ext_control_405",
    name: "Test Chance",
    description: "With chance 1 out of @0 perform next @N",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Test Chance
// Parameters: ${keys}
console.log("Executed legacy action: Test Chance", params);
`;
    }
  },
  {
    id: "ext_control_407",
    name: "Check Question",
    description: "If the user answers yes to a question",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Check Question
// Parameters: ${keys}
console.log("Executed legacy action: Check Question", params);
`;
    }
  },
  {
    id: "ext_control_408",
    name: "Test Expression",
    description: "If an expression is true",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Test Expression
// Parameters: ${keys}
console.log("Executed legacy action: Test Expression", params);
`;
    }
  },
  {
    id: "ext_control_409",
    name: "Check Mouse",
    description: "If @0 mouse button is @Npressed",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Check Mouse
// Parameters: ${keys}
console.log("Executed legacy action: Check Mouse", params);
`;
    }
  },
  {
    id: "ext_control_410",
    name: "Check Grid",
    description: "If instance is aligned with grid",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Check Grid
// Parameters: ${keys}
console.log("Executed legacy action: Check Grid", params);
`;
    }
  },
  {
    id: "ext_control_999_2",
    name: "-----------",
    description: "----------- action",
    category: "control",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: -----------
// Parameters: ${keys}
console.log("Executed legacy action: -----------", params);
`;
    }
  },
  {
    id: "ext_control_999_3",
    name: "Other",
    description: "Other action",
    category: "control",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Other
// Parameters: ${keys}
console.log("Executed legacy action: Other", params);
`;
    }
  },
  {
    id: "ext_control_422",
    name: "Start Block",
    description: "Start of a block",
    category: "control",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAAdUlEQVR4nO3TMQrAIBBE0T1x6hzMOnexziE2KNjH3Q+bwCBjJ/81mlvtKc4LIIAAfwOYzctLACPcu8+RiK14a/ccidiO04hQnESE4xQiFScQ6XgWgcQzCCweRaDxtfO4XiO+AxiPyG0BFoJc6BdUHAEEEECAB10Ei21CITs3AAAAAElFTkSuQmCC",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `{
`;
    }
  },
  {
    id: "ext_control_421",
    name: "Else",
    description: "Else",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `} else {
`;
    }
  },
  {
    id: "ext_control_425",
    name: "Exit Event",
    description: "Exit this event",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `return;
`;
    }
  },
  {
    id: "ext_control_424",
    name: "End Block",
    description: "End of a block",
    category: "control",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAAhklEQVR4nO3OMQ6AIBBEUU5s7cGovQu1h8BgYpSAuANjaCabJTSb/1x0c+f+uPOhLQRIByFE6loRGcD7nbLrsuGAdDQNwEQg8QzAQKDxAjCC6IlXAT2I3vgrAEGMxJsAC2I0/gloIRhxE6CGYMXNgCeCGYcAF4IZhwF/jAACCCCAAAIIIMABj0+MinwE98sAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `}
`;
    }
  },
  {
    id: "ext_control_423",
    name: "Repeat",
    description: "Repeat @0 times",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `for (let _i = 0; _i < (params.times || 1); _i++) {
`;
    }
  },
  {
    id: "ext_control_604",
    name: "Inherited",
    description: "Inherited action",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Inherited
// Parameters: ${keys}
console.log("Executed legacy action: Inherited", params);
`;
    }
  },
  {
    id: "ext_control_999_4",
    name: "----------",
    description: "---------- action",
    category: "control",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: ----------
// Parameters: ${keys}
console.log("Executed legacy action: ----------", params);
`;
    }
  },
  {
    id: "ext_control_999_5",
    name: "Code",
    description: "Code action",
    category: "control",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Code
// Parameters: ${keys}
console.log("Executed legacy action: Code", params);
`;
    }
  },
  {
    id: "ext_control_603",
    name: "Execute Code",
    description: "Execute a piece of code",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return params.code || '';
    }
  },
  {
    id: "ext_control_601",
    name: "Execute_Script",
    description: "Execute_Script action",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Execute_Script
// Parameters: ${keys}
console.log("Executed legacy action: Execute_Script", params);
`;
    }
  },
  {
    id: "ext_control_605",
    name: "Comment",
    description: "@FI@0",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Comment
// Parameters: ${keys}
console.log("Executed legacy action: Comment", params);
`;
    }
  },
  {
    id: "ext_control_999_6",
    name: "-----------------",
    description: "----------------- action",
    category: "control",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: -----------------
// Parameters: ${keys}
console.log("Executed legacy action: -----------------", params);
`;
    }
  },
  {
    id: "ext_control_999_7",
    name: "Variables",
    description: "Variables action",
    category: "control",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Variables
// Parameters: ${keys}
console.log("Executed legacy action: Variables", params);
`;
    }
  },
  {
    id: "ext_control_611",
    name: "Set Variable",
    description: "Set variable @0 to @1",
    category: "control",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAAeklEQVR4nO3WYQqAIAwF4Hd0f3SwXWUnMeyXSFaDzRU95YHKoA9GIgDUyFRczzaqqrqnlGIDiIhrCCDgmwBgOzKu+/3srD9fDjirdwVY18sB4S0YPzSrcfkLLL12b8ErAP++iAggIA3QiiPyGJD6LL8riJ4EEEBAOmAHGc9go5/87NcAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const val = isNaN(Number(params.val)) ? `'` + params.val + `'` : params.val;
      return `this['` + params.name + `'] = ` + (params.rel ? `(this['` + params.name + `'] || 0) + ` : '') + val + `;
`;
    }
  },
  {
    id: "ext_control_612",
    name: "Test Variable",
    description: "If @0 is @N@2 @1",
    category: "control",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `if (!((this['` + params.name + `'] || 0) ` + (params.op || '==') + ` ` + (params.val || 0) + `)) return;
`;
    }
  },
  {
    id: "ext_control_613",
    name: "Draw Variable",
    description: "Draw the value of variable @0",
    category: "control",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAAfElEQVR4nGNgYGD4T0v8nwE/ZAAr+v+a6njVqjzSHPD19WGq4lEHjDpgaDqAgaEFjNHZyHxcYsjidHcANvVUdQCpbLo7gOZRgG4RLjVUyQWkxDXVo2BQOGBkF0SjDhh1wIA5AKSYFphoBwxos5yQAlrDUQeMOmDUAQPuAADHnPKvKTf9NgAAAABJRU5ErkJggg==",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Draw Variable
// Parameters: ${keys}
console.log("Executed legacy action: Draw Variable", params);
`;
    }
  },
  {
    id: "ext_score_999_1",
    name: "Score",
    description: "Score action",
    category: "score",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Score
// Parameters: ${keys}
console.log("Executed legacy action: Score", params);
`;
    }
  },
  {
    id: "ext_score_701",
    name: "Set Score",
    description: "Set the score @rto @0",
    category: "score",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `window.score = ` + (params.rel ? `(window.score || 0) + ` : '') + (params.val || 0) + `;
`;
    }
  },
  {
    id: "ext_score_702",
    name: "Test Score",
    description: "If score is @N@1 @0",
    category: "score",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Test Score
// Parameters: ${keys}
console.log("Executed legacy action: Test Score", params);
`;
    }
  },
  {
    id: "ext_score_703",
    name: "Draw Score",
    description: "Draw the value of score",
    category: "score",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Draw Score
// Parameters: ${keys}
console.log("Executed legacy action: Draw Score", params);
`;
    }
  },
  {
    id: "ext_score_709",
    name: "Show Highscore",
    description: "Show the highscore table",
    category: "score",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Show Highscore
// Parameters: ${keys}
console.log("Executed legacy action: Show Highscore", params);
`;
    }
  },
  {
    id: "ext_score_707",
    name: "Clear Highscore",
    description: "Clear the highscore table",
    category: "score",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Clear Highscore
// Parameters: ${keys}
console.log("Executed legacy action: Clear Highscore", params);
`;
    }
  },
  {
    id: "ext_score_999_2",
    name: "------------",
    description: "------------ action",
    category: "score",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: ------------
// Parameters: ${keys}
console.log("Executed legacy action: ------------", params);
`;
    }
  },
  {
    id: "ext_score_999_3",
    name: "Lives",
    description: "Lives action",
    category: "score",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Lives
// Parameters: ${keys}
console.log("Executed legacy action: Lives", params);
`;
    }
  },
  {
    id: "ext_score_711",
    name: "Set Lives",
    description: "Set lives @rto @0",
    category: "score",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `window.lives = ` + (params.rel ? `(window.lives || 0) + ` : '') + (params.val || 0) + `;
`;
    }
  },
  {
    id: "ext_score_712",
    name: "Test Lives",
    description: "If lives are @N@1 @0",
    category: "score",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Test Lives
// Parameters: ${keys}
console.log("Executed legacy action: Test Lives", params);
`;
    }
  },
  {
    id: "ext_score_713",
    name: "Draw Lives",
    description: "Draw the number of lives",
    category: "score",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Draw Lives
// Parameters: ${keys}
console.log("Executed legacy action: Draw Lives", params);
`;
    }
  },
  {
    id: "ext_score_714",
    name: "Draw Life Images",
    description: "Draw the lives as image",
    category: "score",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Draw Life Images
// Parameters: ${keys}
console.log("Executed legacy action: Draw Life Images", params);
`;
    }
  },
  {
    id: "ext_score_999_4",
    name: "------------",
    description: "------------ action",
    category: "score",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: ------------
// Parameters: ${keys}
console.log("Executed legacy action: ------------", params);
`;
    }
  },
  {
    id: "ext_score_999_5",
    name: "Health",
    description: "Health action",
    category: "score",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Health
// Parameters: ${keys}
console.log("Executed legacy action: Health", params);
`;
    }
  },
  {
    id: "ext_score_721",
    name: "Set Health",
    description: "Set the health @rto @0",
    category: "score",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `this.health = ` + (params.rel ? `(this.health || 0) + ` : '') + (params.val || 0) + `;
`;
    }
  },
  {
    id: "ext_score_722",
    name: "Test Health",
    description: "If health is @N@1 @0",
    category: "score",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Test Health
// Parameters: ${keys}
console.log("Executed legacy action: Test Health", params);
`;
    }
  },
  {
    id: "ext_score_723",
    name: "Draw Health",
    description: "Draw the health bar",
    category: "score",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Draw Health
// Parameters: ${keys}
console.log("Executed legacy action: Draw Health", params);
`;
    }
  },
  {
    id: "ext_score_731",
    name: "Score Caption",
    description: "Set the score caption info",
    category: "score",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Score Caption
// Parameters: ${keys}
console.log("Executed legacy action: Score Caption", params);
`;
    }
  },
  {
    id: "ext_extra_999_1",
    name: "Particles",
    description: "Particles action",
    category: "extra",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Particles
// Parameters: ${keys}
console.log("Executed legacy action: Particles", params);
`;
    }
  },
  {
    id: "ext_extra_820",
    name: "Part_Syst_Create",
    description: "Part_Syst_Create action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Part_Syst_Create
// Parameters: ${keys}
console.log("Executed legacy action: Part_Syst_Create", params);
`;
    }
  },
  {
    id: "ext_extra_821",
    name: "Part_Syst_Destroy",
    description: "Part_Syst_Destroy action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Part_Syst_Destroy
// Parameters: ${keys}
console.log("Executed legacy action: Part_Syst_Destroy", params);
`;
    }
  },
  {
    id: "ext_extra_822",
    name: "Part_Syst_Clear",
    description: "Part_Syst_Clear action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Part_Syst_Clear
// Parameters: ${keys}
console.log("Executed legacy action: Part_Syst_Clear", params);
`;
    }
  },
  {
    id: "ext_extra_825",
    name: "Part_Type_Create_Old",
    description: "Part_Type_Create_Old action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Part_Type_Create_Old
// Parameters: ${keys}
console.log("Executed legacy action: Part_Type_Create_Old", params);
`;
    }
  },
  {
    id: "ext_extra_823",
    name: "Part_Type_Create",
    description: "Part_Type_Create action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Part_Type_Create
// Parameters: ${keys}
console.log("Executed legacy action: Part_Type_Create", params);
`;
    }
  },
  {
    id: "ext_extra_824",
    name: "Part_Type_Color",
    description: "Part_Type_Color action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Part_Type_Color
// Parameters: ${keys}
console.log("Executed legacy action: Part_Type_Color", params);
`;
    }
  },
  {
    id: "ext_extra_826",
    name: "Part_Type_Life",
    description: "Part_Type_Life action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Part_Type_Life
// Parameters: ${keys}
console.log("Executed legacy action: Part_Type_Life", params);
`;
    }
  },
  {
    id: "ext_extra_827",
    name: "Part_Type_Speed",
    description: "Part_Type_Speed action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Part_Type_Speed
// Parameters: ${keys}
console.log("Executed legacy action: Part_Type_Speed", params);
`;
    }
  },
  {
    id: "ext_extra_828",
    name: "Part_Type_Gravity",
    description: "Part_Type_Gravity action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Part_Type_Gravity
// Parameters: ${keys}
console.log("Executed legacy action: Part_Type_Gravity", params);
`;
    }
  },
  {
    id: "ext_extra_829",
    name: "Part_Type_Secondary",
    description: "Part_Type_Secondary action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Part_Type_Secondary
// Parameters: ${keys}
console.log("Executed legacy action: Part_Type_Secondary", params);
`;
    }
  },
  {
    id: "ext_extra_831",
    name: "Part_Emit_Create",
    description: "Part_Emit_Create action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Part_Emit_Create
// Parameters: ${keys}
console.log("Executed legacy action: Part_Emit_Create", params);
`;
    }
  },
  {
    id: "ext_extra_832",
    name: "Part_Emit_Destroy",
    description: "Part_Emit_Destroy action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Part_Emit_Destroy
// Parameters: ${keys}
console.log("Executed legacy action: Part_Emit_Destroy", params);
`;
    }
  },
  {
    id: "ext_extra_833",
    name: "Part_Emit_Burst",
    description: "Part_Emit_Burst action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Part_Emit_Burst
// Parameters: ${keys}
console.log("Executed legacy action: Part_Emit_Burst", params);
`;
    }
  },
  {
    id: "ext_extra_834",
    name: "Part_Emit_Stream",
    description: "Part_Emit_Stream action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Part_Emit_Stream
// Parameters: ${keys}
console.log("Executed legacy action: Part_Emit_Stream", params);
`;
    }
  },
  {
    id: "ext_extra_999_2",
    name: "-----------",
    description: "----------- action",
    category: "extra",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: -----------
// Parameters: ${keys}
console.log("Executed legacy action: -----------", params);
`;
    }
  },
  {
    id: "ext_extra_999_3",
    name: "CD",
    description: "CD action",
    category: "extra",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: CD
// Parameters: ${keys}
console.log("Executed legacy action: CD", params);
`;
    }
  },
  {
    id: "ext_extra_808",
    name: "CD_Play",
    description: "CD_Play action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: CD_Play
// Parameters: ${keys}
console.log("Executed legacy action: CD_Play", params);
`;
    }
  },
  {
    id: "ext_extra_809",
    name: "CD_Stop",
    description: "CD_Stop action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: CD_Stop
// Parameters: ${keys}
console.log("Executed legacy action: CD_Stop", params);
`;
    }
  },
  {
    id: "ext_extra_810",
    name: "CD_Pause",
    description: "CD_Pause action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: CD_Pause
// Parameters: ${keys}
console.log("Executed legacy action: CD_Pause", params);
`;
    }
  },
  {
    id: "ext_extra_811",
    name: "CD_Resume",
    description: "CD_Resume action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: CD_Resume
// Parameters: ${keys}
console.log("Executed legacy action: CD_Resume", params);
`;
    }
  },
  {
    id: "ext_extra_812",
    name: "CD_If_Exists",
    description: "CD_If_Exists action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: CD_If_Exists
// Parameters: ${keys}
console.log("Executed legacy action: CD_If_Exists", params);
`;
    }
  },
  {
    id: "ext_extra_813",
    name: "CD_If_Playing",
    description: "CD_If_Playing action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: CD_If_Playing
// Parameters: ${keys}
console.log("Executed legacy action: CD_If_Playing", params);
`;
    }
  },
  {
    id: "ext_extra_999_4",
    name: "---------------",
    description: "--------------- action",
    category: "extra",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: ---------------
// Parameters: ${keys}
console.log("Executed legacy action: ---------------", params);
`;
    }
  },
  {
    id: "ext_extra_999_5",
    name: "Other",
    description: "Other action",
    category: "extra",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Other
// Parameters: ${keys}
console.log("Executed legacy action: Other", params);
`;
    }
  },
  {
    id: "ext_extra_801",
    name: "Set_Mouse",
    description: "Set_Mouse action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Set_Mouse
// Parameters: ${keys}
console.log("Executed legacy action: Set_Mouse", params);
`;
    }
  },
  {
    id: "ext_extra_807",
    name: "Open_Webpage",
    description: "Open_Webpage action",
    category: "extra",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Open_Webpage
// Parameters: ${keys}
console.log("Executed legacy action: Open_Webpage", params);
`;
    }
  },
  {
    id: "ext_draw_999_1",
    name: "--------------",
    description: "-------------- action",
    category: "draw",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: --------------
// Parameters: ${keys}
console.log("Executed legacy action: --------------", params);
`;
    }
  },
  {
    id: "ext_draw_999_2",
    name: "Drawing",
    description: "Drawing action",
    category: "draw",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Drawing
// Parameters: ${keys}
console.log("Executed legacy action: Drawing", params);
`;
    }
  },
  {
    id: "ext_draw_501",
    name: "Draw Sprite",
    description: "Draw sprite @0",
    category: "draw",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Draw Sprite
if (window.assets && window.assets[params.spr]) {
  var img = window.assets[params.spr];
  var fw = img.width / (params.subimages || 1);
  ctx.drawImage(img, (params.subimage || 0) * fw, 0, fw, img.height, (params.rel ? this.x : 0) + (params.x || 0), (params.rel ? this.y : 0) + (params.y || 0), fw, img.height);
}
`;
    }
  },
  {
    id: "ext_draw_502",
    name: "Draw Background",
    description: "Draw background @0",
    category: "draw",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Draw Background
// Parameters: ${keys}
console.log("Executed legacy action: Draw Background", params);
`;
    }
  },
  {
    id: "ext_draw_514",
    name: "Draw Text",
    description: "Draw a text",
    category: "draw",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Draw Text
ctx.fillStyle = params.color || '#FFFFFF';
ctx.fillText(params.text || '', (params.rel ? this.x : 0) + (params.x || 0), (params.rel ? this.y : 0) + (params.y || 0));
`;
    }
  },
  {
    id: "ext_draw_519",
    name: "Draw_Text_Scaled",
    description: "Draw_Text_Scaled action",
    category: "draw",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Draw_Text_Scaled
// Parameters: ${keys}
console.log("Executed legacy action: Draw_Text_Scaled", params);
`;
    }
  },
  {
    id: "ext_draw_511",
    name: "Draw Rectangle",
    description: "Draw a rectangle",
    category: "draw",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      return `// Draw Rectangle
ctx.fillStyle = params.color || '#FFFFFF';
ctx.fillRect((params.rel ? this.x : 0) + (params.x1 || 0), (params.rel ? this.y : 0) + (params.y1 || 0), (params.x2 || 0) - (params.x1 || 0), (params.y2 || 0) - (params.y1 || 0));
`;
    }
  },
  {
    id: "ext_draw_516",
    name: "Draw_Gradient_Hor",
    description: "Draw_Gradient_Hor action",
    category: "draw",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Draw_Gradient_Hor
// Parameters: ${keys}
console.log("Executed legacy action: Draw_Gradient_Hor", params);
`;
    }
  },
  {
    id: "ext_draw_517",
    name: "Draw_Gradient_Vert",
    description: "Draw_Gradient_Vert action",
    category: "draw",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Draw_Gradient_Vert
// Parameters: ${keys}
console.log("Executed legacy action: Draw_Gradient_Vert", params);
`;
    }
  },
  {
    id: "ext_draw_512",
    name: "Draw Ellipse",
    description: "Draw an ellipse",
    category: "draw",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Draw Ellipse
// Parameters: ${keys}
console.log("Executed legacy action: Draw Ellipse", params);
`;
    }
  },
  {
    id: "ext_draw_518",
    name: "Draw_Ellipse_Gradient",
    description: "Draw_Ellipse_Gradient action",
    category: "draw",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Draw_Ellipse_Gradient
// Parameters: ${keys}
console.log("Executed legacy action: Draw_Ellipse_Gradient", params);
`;
    }
  },
  {
    id: "ext_draw_513",
    name: "Draw Line",
    description: "Draw a line",
    category: "draw",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Draw Line
// Parameters: ${keys}
console.log("Executed legacy action: Draw Line", params);
`;
    }
  },
  {
    id: "ext_draw_515",
    name: "Draw Arrow",
    description: "Draw an arrow",
    category: "draw",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Draw Arrow
// Parameters: ${keys}
console.log("Executed legacy action: Draw Arrow", params);
`;
    }
  },
  {
    id: "ext_draw_999_3",
    name: "----------------",
    description: "---------------- action",
    category: "draw",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: ----------------
// Parameters: ${keys}
console.log("Executed legacy action: ----------------", params);
`;
    }
  },
  {
    id: "ext_draw_999_4",
    name: "Settings",
    description: "Settings action",
    category: "draw",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Settings
// Parameters: ${keys}
console.log("Executed legacy action: Settings", params);
`;
    }
  },
  {
    id: "ext_draw_524",
    name: "Set Color",
    description: "Set the color",
    category: "draw",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Set Color
// Parameters: ${keys}
console.log("Executed legacy action: Set Color", params);
`;
    }
  },
  {
    id: "ext_draw_526",
    name: "Set_Font",
    description: "Set_Font action",
    category: "draw",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Set_Font
// Parameters: ${keys}
console.log("Executed legacy action: Set_Font", params);
`;
    }
  },
  {
    id: "ext_draw_531",
    name: "Set Full Screen",
    description: "Change full screen mode",
    category: "draw",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Set Full Screen
// Parameters: ${keys}
console.log("Executed legacy action: Set Full Screen", params);
`;
    }
  },
  {
    id: "ext_draw_999_5",
    name: "----------------",
    description: "---------------- action",
    category: "draw",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: ----------------
// Parameters: ${keys}
console.log("Executed legacy action: ----------------", params);
`;
    }
  },
  {
    id: "ext_draw_999_6",
    name: "Other",
    description: "Other action",
    category: "draw",
    iconUrl: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAATUlEQVR4nO3WzQkAIAyD0Qzf2bpWRQfwBwwifA255fCOlaRytjSPxshwEXEGyMyrBQAAAAAAAAAAAPAXoI8d3QY8fctXA3cAAADwHNAA1Q8YAMbbG0AAAAAASUVORK5CYII=",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Other
// Parameters: ${keys}
console.log("Executed legacy action: Other", params);
`;
    }
  },
  {
    id: "ext_draw_802",
    name: "Take_Snapshot",
    description: "Take_Snapshot action",
    category: "draw",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Take_Snapshot
// Parameters: ${keys}
console.log("Executed legacy action: Take_Snapshot", params);
`;
    }
  },
  {
    id: "ext_draw_532",
    name: "Effect",
    description: "Effect action",
    category: "draw",
    iconUrl: "data:image/png;base64,",
    params: [],
    generateCode: (params: Record<string, any>) => {
      const keys = Object.keys(params).map(k => `${k}: ${params[k]}`).join(', ');
      return `// GameMaker Legacy Action: Effect
// Parameters: ${keys}
console.log("Executed legacy action: Effect", params);
`;
    }
  },
];
